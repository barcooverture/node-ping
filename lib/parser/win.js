...
WinParser.prototype._processBody = function (line) {
    ...
    // XXX: Below expectation means pingable or not
    var expectKVPLength = null;

    // XXX: Output format for pinging ipv6 or ipv4 address is different
    if (this._pingConfig.v6) {
        expectKVPLength = 1;
    } else {
        expectKVPLength = 3;
    }

    if (kvps.length >= expectKVPLength) {
